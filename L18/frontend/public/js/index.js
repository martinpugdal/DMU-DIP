...
let diceData = get(getAPIURL() + "/dice?roomID=" + roomID, sessionID).then(
	(values) => {
		return values;
	}
);
...
function updateScoreTable(first = false) {
	scoreTable.forEach((scoreField, index) => {
		if (
			diceData.results[index] === -1 &&
			diceData.combinations[index] !== 0 &&
			first === false
		) {
			scoreField.textContent = diceData.combinations[index];
		} else if (first === true && diceData.results[index] === -1) {
			scoreField.textContent = diceData.combinations[index];
		}
	});
}
...
async function roll() {
	...
	const res = await post(getAPIURL() + "/dice/roll", obj, sessionID);
	if (res.success === true) {
		diceData = res.dice;
	} else {
